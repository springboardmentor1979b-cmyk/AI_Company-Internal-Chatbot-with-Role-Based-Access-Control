...
with st.sidebar:
    # Dragon brand
    dragon_small = DRAGON_SVG.format(size=38)
    st.markdown(f"""
    <div style="padding:1.2rem 1rem 0.8rem;border-bottom:1px solid var(--border);
      display:flex;align-items:center;gap:10px;">
      <div style="animation:dragon-breathe 4s ease-in-out infinite;
        filter:drop-shadow(0 0 8px rgba(124,106,247,0.7));">
        {dragon_small}
      </div>
      <div>
...
if not st.session_state["token"]:
    ...
    # Pre-render SVGs
    dragon_login = DRAGON_SVG.format(size=90)
    dragon_chat  = DRAGON_SVG.format(size=60)
    ...
    <div style="text-align:center;padding:2rem 0 1.5rem;animation:fadeSlideUp 0.8s ease-out;">
      <div class="dragon-logo" style="display:inline-block;margin-bottom:1rem;">
        {dragon_login}
      </div>
...
# ══════════════════════════════════════════
    ...
    if not st.session_state["chat_history"]:
        ...
        <div style="text-align:center;padding:3rem 0;animation:fadeSlideUp 0.6s ease-out;">
          <div style="filter:drop-shadow(0 0 15px rgba(124,106,247,0.4));display:inline-block;margin-bottom:1rem;">
            {dragon_chat}
          </div>
